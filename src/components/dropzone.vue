<template>
	<VueFileAgent ref="vueFileAgent" :theme="'list'" :multiple="true" :editable="true" :deletable="true" :meta="true" :accept="'image/*,.zip'" :maxSize="'10MB'" :maxFiles="14" :helpText="'Choose images or zip files'" :errorText="{
        type: 'Invalid file type. Only images or zip Allowed',
        size: 'Files should not exceed 10MB in size'
      }" @select="uploadFiles(customerId)" @upload="uploadedEvent('upload', $event)" @upload:error="uploadedEvent('upload:error', $event)" @beforedelete="onBeforeDelete($event)" @delete="fileDeleted($event)" v-model="fileRecords" />
	</div>
</template>
